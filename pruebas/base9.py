...
def predecir_mantenimiento(datos):
    datos_normalizados = {
        "kilometraje": normalizar(datos['kilometraje'], 0, 1000000),  # Se ajustó a 1,000,000 km de vida útil
        "temperatura_motor": normalizar(datos['temperatura_motor'], 80, 95),  # Se ajustó a temperatura óptima
        "horas_operacion": normalizar(datos['horas_operacion'], 0, 3000),  # Ajustado a operación anual esperada
        "historial_eventos": normalizar(datos['historial_eventos'], 0, 5),  # Reducción del rango por recomendación
        "rpm": normalizar(datos['rpm'], 500, 5000),  # Rango óptimo de operación
        "masas_homocineticas": normalizar(datos['masas_homocineticas'], 0, 10),  # Rango estándar de desgaste
        "desgaste_frenos": normalizar(datos['desgaste_frenos'], 0, 10)  # Revisión cada 20,000 km
    }
...
# **Nuevos datos basados en especificaciones de camiones urbanos en Aguascalientes**
datos_camion_urbano = {
...
# Ejecutar la predicción con los valores ajustados
if __name__ == "__main__":
    predecir_mantenimiento(datos_camion_urbano)
